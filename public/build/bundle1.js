/* Cafe 0.0.91 Sat Mar 23 2013 18:53:26 GMT+0200 (EET) */

// Generated by CoffeeScript 1.3.1
(function() {
  "Simple common js bootstrapper.\nInspired by stitch.";

  var Q, add, cache, diranme, doc, expand, modules, partial, pre, q, queues, rem, require, _i, _len,
  ...
  if (!this.require) {
    ...
    window.bootstrapper.queues = Q;
    window.bootstrapper.put_to_queue = function(q, f) {
      var fs;
      if (__indexOf.call(queues, q) >= 0) {
        fs = Array.isArray(f) ? f : [f];
    ...
    window.bootstrapper.run_queue = function(qname) {
      var f, _results;
      if (__indexOf.call(queues, qname) >= 0) {
        _results = [];
    ...
    window.bootstrapper.run_init_queue = function() {
      return window.bootstrapper.run_queue('init_queue');
    };
    doc = window.document;
    ...
    if (doc.readyState === 'complete') {
      ...
      doc[add](pre + 'DOMContentLoaded', function() {
        return window.bootstrapper.run_queue('document_ready_queue');
      });
      window[add](pre + 'load', function() {
        return window.bootstrapper.run_queue('document_loaded_queue');
      });
    }
    if (!window.bootstrapper) {
      doc = window.document;
    ...
    partial = function(fn) {
      ...
      partial_args.shift();
      return function() {
        var a, arg, i, new_args, _j, _len1, _ref;
        _ref = [[], 0], new_args = _ref[0], arg = _ref[1];
        for (i = _j = 0, _len1 = partial_args.length; _j < _len1; i = ++_j) {
          a = partial_args[i];
    ...
    expand = function(root, name) {
      var i, part, parts, results, _j, _ref;
      results = [];
      if (/^\.\.?(\/|$)/.test(name)) {
      ...
      for (i = _j = 0, _ref = parts.length - 1; 0 <= _ref ? _j <= _ref : _j >= _ref; i = 0 <= _ref ? ++_j : --_j) {
        part = parts[i];
        if (part === '..') {
          results.pop();
        } else if (part !== '.' && part !== '') {
...
(function() {
  var BUILTIN, CELLS, DNA_DATATYPES, DNA_EXTEND, DNA_ID_PREFIX, DNA_PRIMITIVES, DNA_SUBSCRIBE, FLOAT, FUNCTION, HASHMAP, INTEGER, KEYWORD, Math, NAN, NESTED_EXPR, NULL, PARTIAL_FN, QUOTED_NESTED_EXPR, STRING, THIS, VECTOR, bind_handlers_to_event, bool, complement, compose3, cont_m, cont_t, debug, default_handlers_cont, devnull, dispatch_handler, dispatch_impl, distinct, domonad, error, find_cell, fun_with_meta, get_arity, get_cell, get_create_cell, get_create_cell_by_id, get_default_protocols, get_method_ns, get_primitive_value_handler, get_protocol, get_value_handler, info, is_array, is_async, is_function, is_just_function, is_nested_expr, is_null, is_object, is_partial_function, is_value, lift, lift_async, lift_sync, logger_m, logger_t, make_extended_node, make_monadized_handler, make_nested_expr, make_subscribed_node, maybe_m, maybe_t, parse_genome, partial, process_ast_handler_node, process_meta, process_vector, register_protocol_impl, save_cell, synthesize_cell, synthesize_node, warn, _ref, _ref1, _ref2, _ref3,
    __slice = [].slice,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };
  ...
  _ref = require('libprotein'), partial = _ref.partial, is_array = _ref.is_array, is_object = _ref.is_object, bool = _ref.bool, complement = _ref.complement, compose3 = _ref.compose3, distinct = _ref.distinct;
  ...
  _ref3 = dispatch_impl('ILogger', 'DNA'), info = _ref3.info, warn = _ref3.warn, error = _ref3.error, debug = _ref3.debug;
  ...
  make_subscribed_node = function(dom_parser, node) {
    ...
    return genes.map(function(gene) {
      return gene.events.map(partial(bind_handlers_to_event, dom_parser, cell, gene.handlers.map(partial(make_monadized_handler, dom_parser, cell, default_handlers_cont))));
    });
  };
  ...
  synthesize_node = function(dom_parser) {
    ...
    return subscribed_nodes.map(partial(make_subscribed_node, dom_parser));
  };
...
(function() {
  ...
  _ref = lp.dispatch_impl('ILogger', 'IDom'), info = _ref.info, warn = _ref.warn, debug = _ref.debug, error = _ref.error, notice = _ref.notice;

  IDom = [
    ['setContent', ['new_content']], ['setValue', ['new_value']], ['setText', ['new_text']], ['getValue', []], ['getValueAsync', ['cont'], 'async'], ['alert', ['msg']], ['click', ['handler']], ['keyDown', ['handler']], ['say', ['msgs']], ['appendContent', ['content']], ['kill', []], ['stop_event', ['e']], ['setAttr', ['attr']], ['dbclick', ['e']], ['focusout', ['e']], ['focus', []], ['get_by_attr', ['attr']], ['get_by_id', ['id']], ['getData', ['attr', 'node']], ['get_id', ['node']], ['on_dom_ready', ['f']], ['one', ['sel']], ['document', []], ['get_root_node', []], ['add_event_listener', ['event_name', 'handler']], ['on_change', ['f']], ['proxylog', ['a']], [
      'debug', ['a', 'b'], {
        vargs: true
      }
    ]
  ...
  jqidom = function(node) {
    ...
    return {
      ...
      keyDown: function(handler) {
        return $node.on('keydown', handler);
      },
...
/*ZB:  libprotocol/node_modules/console-logger/index */
'node_modules/console-logger/index': function(exports, require, module) {(function() {
(function() {
  var DEBUG, ERROR, INFO, LOGCFG, LOG_LEVELS, NOTICE, UNK_NS, WARN, and_, bool, bt, e, get_namespaced_logger, is_array, is_object, log, log_level_enabled, log_ns_enabled, or_, partial, say, _ref,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __slice = [].slice;
...
// Generated by CoffeeScript 1.3.1
(function() {
  var DEBUG, ERROR, INFO, LOGCFG, LOG_LEVELS, NOTICE, UNK_NS, WARN, and_, bool, bt, get_namespaced_logger, is_array, is_object, log, log_level_enabled, log_ns_enabled, or_, partial, say, _ref,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __slice = [].slice;
...
/*ZB:  libprotocol/node_modules/libprotein/index */
'node_modules/libprotein/index': function(exports, require, module) {(function() {
(function() {
  var X,
    __slice = [].slice,
    __hasProp = {}.hasOwnProperty;
...
/*ZB:  libmonad/node_modules/console-logger/index */
'node_modules/console-logger/index': function(exports, require, module) {(function() {
(function() {
  var DEBUG, ERROR, INFO, LOGCFG, LOG_LEVELS, NOTICE, UNK_NS, WARN, and_, bool, bt, e, get_namespaced_logger, is_array, is_object, log, log_level_enabled, log_ns_enabled, or_, partial, say, _ref,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
...
/*ZB:  libmonad/node_modules/libprotein/index */
'node_modules/libprotein/index': function(exports, require, module) {(function() {
(function() {
  var X,
    __slice = [].slice,
